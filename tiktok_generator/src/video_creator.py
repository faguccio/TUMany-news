...
from .video_prepare import prepare_background, create_video_with_ffmpeg
...
class VideoCreator:
    ...
    def download_video(self, folder='background'):
        if self.args.images:
            logger.info(f"Not downloading video since images are used")
            return
    ...
    def select_background(self):
        ...
        if self.args.images:
            audio_info = get_info(self.mp3_file, kind='audio')
            audio_duration = int(round(audio_info.get('duration'), 0))
            self.mp4_background = create_video_with_ffmpeg(self.args.images, "./background/sample_background.mp4", audio_duration)
            return self.mp4_background
